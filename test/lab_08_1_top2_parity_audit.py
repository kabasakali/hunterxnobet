"""
test/lab_08_1_top2_parity_audit.py
==================================
LAB-08.1: TOP-2 BENCHMARK PARITY & ALPHA ATTRIBUTION AUDIT

Amaç:
1. LAB-E (%64.67 CAGR / 1.070.946 TL) ile REF (100/0) arasında %100 birebir
   günlük bakiye ve karar parity'sini (eşitliğini) sağlamak ve kanıtlamak.
2. Hunter ve Momentum motorlarının gerçek #1 ve #2 şampiyonlarını motor kurallarına
   (asgari tutma, valör, T-1) tam sadık kalarak %90/#1 + %10/#2 şeklinde harmanlamak.
3. Top-2'nin gerçekten net alfa katıp katmadığını gün gün kuruşu kuruşuna
   atfetmek (Alpha Attribution).

Kurallar:
- Katı T-1 veri kullanımı
- 13:00 karar kesimi
- Gerçek fon valörleri (T+1 / T+2)
- T0 PPZ nakit neması
- 2025 Kör Validasyon ve 2026 Saf OOS ayrımı
- Sıfır sızıntı garantisi
"""

import os
import sys
import numpy as np
import pandas as pd

base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, base_dir)

from harmony_v1_motor import HarmonyMotor
from meta_allocator_motor import MetaAllocatorMotor
from harmony_v2_2_golden import HarmonyV22GoldenMotor

class Lab081Top2ParityEngine:
    def __init__(self, baslangic_sermayesi: float = 100_000.0, kayma_pct: float = 0.0005):
        self.baslangic_sermayesi = baslangic_sermayesi
        self.kayma_pct = kayma_pct
        
        self.golden = HarmonyV22GoldenMotor(baslangic_sermayesi=baslangic_sermayesi, kayma_pct=kayma_pct)
        self.tarihler = self.golden.tarihler
        self.t0_ret_serisi = self.golden.t0_ret_serisi
        
        self.r_mom = self.golden.r_mom
        self.r_h6 = self.golden.r_h6
        self.r_dd = self.golden.r_dd
        self.r_sfb = self.golden.r_sfb
        
        self.market_breadth = self.golden.market_breadth
        self.top5_avg_r20 = self.golden.top5_avg_r20
        self.r20_df = self.golden.r20_df
        self.sfb_aktif_fon = self.golden.sfb_aktif_fon
        self.h6_aktif_fon = self.golden.h6_aktif_fon
        
        # Temel V1 Motoru
        self.base_v1 = self.golden.base_motor.base_motor
        self.fiyat_df = self.base_v1.fiyat_df
        self.skor_df = self.base_v1.skor_df
        self.fon_gunluk_ret = self.fiyat_df.pct_change().fillna(0.0)
        
        # Hunter Top-2 Fonunu ve Getirisini Hesapla
        self._hunter_top2_ve_sfb_top2_hazirla()

    def _hunter_top2_ve_sfb_top2_hazirla(self):
        """
        Hunter ve SFB alt motorlarının #2 şampiyonlarını ve getirilerini
        birebir aynı T-1 filtreleri ve asgari tutma kurallarıyla hesaplar.
        """
        tarih_listesi = list(self.skor_df.index)
        
        # 1. Hunter #2 Fonu (6 Günlük Trend Korumalı)
        hunter_top2_fon = {}
        r_h_top2 = {}
        eldeki_h2 = "PPZ"
        elde_gun_h2 = 0
        
        # 2. SFB #2 Fonu (2 Günlük İvme Korumalı)
        sfb_top2_fon = {}
        r_s_top2 = {}
        eldeki_s2 = "PPZ"
        elde_gun_s2 = 0
        
        for dt in tarih_listesi:
            skorlar = self.skor_df.loc[dt].dropna().sort_values(ascending=False)
            skorlar = skorlar[[c for c in skorlar.index if c not in ['PPZ', 'date', 'tarih']]]
            
            # Hunter #2: 2. sıradaki fon (Top-2)
            if len(skorlar) >= 2:
                aday_h2 = skorlar.index[1]
                s1 = skorlar.iloc[0]
                s2 = skorlar.iloc[1]
                r20_2 = self.r20_df.at[dt, aday_h2] if aday_h2 in self.r20_df.columns and pd.notna(self.r20_df.at[dt, aday_h2]) else 0.0
                
                # Kalite Filtresi: 2. fon güçlü mü?
                if s2 >= 0.70 * s1 and r20_2 >= 0.020:
                    hedef_h2 = aday_h2
                else:
                    hedef_h2 = skorlar.index[0] # Yeterince güçlü değilse 1. fona eşitlenir
            elif len(skorlar) == 1:
                hedef_h2 = skorlar.index[0]
            else:
                hedef_h2 = "PPZ"
                
            if hedef_h2 != eldeki_h2:
                if elde_gun_h2 >= 6:
                    eldeki_h2 = hedef_h2
                    elde_gun_h2 = 1
                else:
                    elde_gun_h2 += 1
            else:
                elde_gun_h2 += 1
                
            hunter_top2_fon[dt] = eldeki_h2
            r_h_top2[dt] = self.fon_gunluk_ret.at[dt, eldeki_h2] if eldeki_h2 in self.fon_gunluk_ret.columns else 0.0
            
            # SFB #2
            if len(skorlar) >= 2:
                hedef_s2 = skorlar.index[1] if (skorlar.iloc[1] >= 0.70 * skorlar.iloc[0]) else skorlar.index[0]
            elif len(skorlar) == 1:
                hedef_s2 = skorlar.index[0]
            else:
                hedef_s2 = "PPZ"
                
            if hedef_s2 != eldeki_s2:
                if elde_gun_s2 >= 2:
                    eldeki_s2 = hedef_s2
                    elde_gun_s2 = 1
                else:
                    elde_gun_s2 += 1
            else:
                elde_gun_s2 += 1
                
            sfb_top2_fon[dt] = eldeki_s2
            r_s_top2[dt] = self.fon_gunluk_ret.at[dt, eldeki_s2] if eldeki_s2 in self.fon_gunluk_ret.columns else 0.0
            
        self.r_h_top2 = r_h_top2
        self.r_s_top2 = r_s_top2

    def simule_et_parity(self, w1: float = 1.0, w2: float = 0.0) -> dict:
        sermaye = self.baslangic_sermayesi
        rolling_window = []
        
        tarih_dilimi = self.tarihler[65:]
        gunluk_kayitlar = []
        gecisler = []
        
        aktif_motor = "HUNTER"
        motor_elde_gun = 0
        savunma_modu = False
        takas_kalan_gun = 0
        
        # Attribution Takibi
        top2_fark_havuzu_tl = 0.0
        top2_ekstra_kazanc_tl = 0.0
        top2_kayip_tl = 0.0
        
        for idx, dt in enumerate(tarih_dilimi):
            rm = self.r_mom.get(dt, 0.0)
            rh = self.r_h6.get(dt, 0.0)
            rdd = self.r_dd.get(dt, 0.0)
            rt0 = self.t0_ret_serisi.loc[dt]
            
            # #2 Fon getirileri
            rh2 = self.r_h_top2.get(dt, rh)
            rm2 = self.r_s_top2.get(dt, rm)
            
            breadth = self.market_breadth.get(dt, 0.0)
            top5 = self.top5_avg_r20.get(dt, 0.0)
            
            f_sfb = self.sfb_aktif_fon.get(dt, "PPZ")
            f_h6 = self.h6_aktif_fon.get(dt, "PPZ")
            r20_s = self.r20_df.at[dt, f_sfb] if f_sfb in self.r20_df.columns and pd.notna(self.r20_df.at[dt, f_sfb]) else 0.0
            r20_h = self.r20_df.at[dt, f_h6] if f_h6 in self.r20_df.columns and pd.notna(self.r20_df.at[dt, f_h6]) else 0.0
            spread = r20_s - r20_h
            
            # LAB-E Rolling Peak 60d
            rolling_window.append(sermaye)
            if len(rolling_window) > 60:
                rolling_window.pop(0)
            local_peak = max(rolling_window)
            local_dd = (local_peak - sermaye) / (local_peak + 1e-8)
            
            # Savunma Mantığı (LAB-E Referansıyla BİREBİR AYNI)
            if local_dd >= 0.040 or breadth <= 0.25 or top5 <= 0.015:
                savunma_modu = True
            elif local_dd <= 0.025 and (breadth >= 0.40 or top5 >= 0.03):
                savunma_modu = False
                
            nobetci_onay = (spread >= 0.020 and r20_s >= 0.03 and breadth >= 0.45)
            if savunma_modu:
                hedef_motor = "DD_AWARE"
            elif nobetci_onay:
                hedef_motor = "MOMENTUM"
            else:
                hedef_motor = "HUNTER"
                
            asgari_sure = 5 if aktif_motor == "DD_AWARE" else 3
            acil_fren = (hedef_motor == "DD_AWARE" and local_dd >= 0.045)
            hizli_cikis = (aktif_motor == "DD_AWARE" and breadth >= 0.48 and spread >= 0.025)
            
            t_eksi_1_str = tarih_dilimi[idx-1].strftime("%Y-%m-%d") if idx > 0 else dt.strftime("%Y-%m-%d")
            
            if hedef_motor != aktif_motor and takas_kalan_gun == 0:
                if motor_elde_gun >= asgari_sure or acil_fren or hizli_cikis:
                    sermaye *= (1.0 - self.kayma_pct)
                    val_satis = 2 if aktif_motor == "MOMENTUM" else 1
                    takas_kalan_gun = val_satis
                    
                    gecisler.append({
                        "tarih": dt.strftime("%Y-%m-%d"),
                        "t_eksi_1": t_eksi_1_str,
                        "eski": aktif_motor,
                        "yeni": hedef_motor,
                        "bakiye": round(sermaye, 2),
                        "yil": dt.year
                    })
                    
                    aktif_motor = hedef_motor
                    motor_elde_gun = 1
                else:
                    motor_elde_gun += 1
            else:
                motor_elde_gun += 1
                
            # ─────────────────────────────────────────────────────────────────
            # GETİRİ HESABI (W1 Lider + W2 İkinci)
            # ─────────────────────────────────────────────────────────────────
            if takas_kalan_gun > 0:
                sermaye *= (1 + rt0)
                takas_kalan_gun -= 1
                g_ret = rt0
            else:
                if aktif_motor == "HUNTER":
                    g_ret = (w1 * rh) + (w2 * rh2)
                    alfa_fark = (g_ret - rh) * sermaye
                elif aktif_motor == "MOMENTUM":
                    g_ret = (w1 * rm) + (w2 * rm2)
                    alfa_fark = (g_ret - rm) * sermaye
                else:
                    g_ret = rdd
                    alfa_fark = 0.0
                    
                if alfa_fark > 0:
                    top2_ekstra_kazanc_tl += alfa_fark
                elif alfa_fark < 0:
                    top2_kayip_tl += abs(alfa_fark)
                top2_fark_havuzu_tl += alfa_fark
                
                sermaye *= (1 + g_ret)
                
            gunluk_kayitlar.append({
                "tarih": dt,
                "bakiye": sermaye,
                "aktif_motor": aktif_motor,
                "gunluk_ret": g_ret,
                "yil": dt.year
            })
            
        df = pd.DataFrame(gunluk_kayitlar).set_index("tarih")
        
        gun_sayisi = len(df)
        yil_sayisi = gun_sayisi / 252.0
        cagr = ((sermaye / self.baslangic_sermayesi) ** (1.0 / yil_sayisi) - 1.0) * 100.0
        
        df["peak"] = df["bakiye"].cummax()
        df["dd"] = (df["bakiye"] - df["peak"]) / df["peak"]
        mdd = df["dd"].min() * 100.0
        
        daily_rets = df["gunluk_ret"]
        mean_ret = daily_rets.mean()
        std_ret = daily_rets.std()
        sharpe = (mean_ret / (std_ret + 1e-9)) * np.sqrt(252)
        
        neg_rets = daily_rets[daily_rets < 0]
        downside_std = neg_rets.std() if len(neg_rets) > 0 else 1e-9
        sortino = (mean_ret / (downside_std + 1e-9)) * np.sqrt(252)
        
        calmar = (cagr / abs(mdd)) if abs(mdd) > 0 else 0.0
        
        gains = daily_rets[daily_rets > 0].sum()
        losses = abs(daily_rets[daily_rets < 0].sum())
        pf = gains / losses if losses > 0 else np.nan
        
        yillik_ret = {}
        for y in sorted(df["yil"].unique()):
            sub = df[df["yil"] == y]
            r = (sub["bakiye"].iloc[-1] - sub["bakiye"].iloc[0]) / sub["bakiye"].iloc[0] * 100.0
            yillik_ret[y] = r
            
        return {
            "w1": w1,
            "w2": w2,
            "cagr": cagr,
            "son_bakiye": sermaye,
            "mdd": mdd,
            "sharpe": sharpe,
            "sortino": sortino,
            "calmar": calmar,
            "pf": pf,
            "toplam_gecis": len(gecisler),
            "top2_ekstra_kazanc_tl": top2_ekstra_kazanc_tl,
            "top2_kayip_tl": top2_kayip_tl,
            "net_top2_alfa_tl": top2_fark_havuzu_tl,
            "yillik_ret": yillik_ret,
            "df": df
        }

def run_parity_and_attribution_audit():
    engine = Lab081Top2ParityEngine()
    
    agirliklar = [
        (1.0, 0.0, "LAB-E REF (100/0)"),
        (0.9, 0.1, "LAB-08.1A (90/10)"),
        (0.8, 0.2, "LAB-08.1B (80/20)"),
        (0.7, 0.3, "LAB-08.1C (70/30)"),
        (0.6, 0.4, "LAB-08.1D (60/40)"),
        (0.5, 0.5, "LAB-08.1E (50/50)")
    ]
    
    sonuclar = []
    for w1, w2, lbl in agirliklar:
        res = engine.simule_et_parity(w1=w1, w2=w2)
        res["label"] = lbl
        sonuclar.append(res)
        
    ref = sonuclar[0]
    
    print("=" * 115)
    print("LAB-08.1: TOP-2 BENCHMARK PARITY & ALPHA ATTRIBUTION RESMI AUDIT MATRISI")
    print("=" * 115)
    
    hdr = f"{'Varyant':<20} | {'CAGR':<8} | {'Son Bakiye':<14} | {'MaxDD':<8} | {'Sharpe':<7} | {'Sortino':<8} | {'Calmar':<7} | {'PF':<6} | {'Net Alfa (TL)'}"
    print(hdr)
    print("-" * 115)
    
    for s in sonuclar:
        net_alfa_str = f"{s['net_top2_alfa_tl']:>+12,.2f} TL" if s['w2'] > 0 else "0.00 TL (REF)"
        print(f"{s['label']:<20} | %{s['cagr']:<6.2f} | {s['son_bakiye']:<11,.0f} TL | %{s['mdd']:<6.2f} | {s['sharpe']:<7.2f} | {s['sortino']:<8.2f} | {s['calmar']:<7.2f} | {s['pf']:<6.2f} | {net_alfa_str}")
        
    print("-" * 115)
    print("YILLARA GORE DETAYLI GETIRI VE KOR/OOS DOKUMU:")
    print("-" * 115)
    
    y_hdr = f"{'Varyant':<20} | {'2021':<8} | {'2022':<8} | {'2023':<8} | {'2024':<8} | {'2025 Blind':<11} | {'2026 OOS':<10}"
    print(y_hdr)
    print("-" * 115)
    
    for s in sonuclar:
        r21 = f"%{s['yillik_ret'].get(2021, 0):+.1f}"
        r22 = f"%{s['yillik_ret'].get(2022, 0):+.1f}"
        r23 = f"%{s['yillik_ret'].get(2023, 0):+.1f}"
        r24 = f"%{s['yillik_ret'].get(2024, 0):+.1f}"
        r25 = f"%{s['yillik_ret'].get(2025, 0):+.1f}"
        r26 = f"%{s['yillik_ret'].get(2026, 0):+.1f}"
        print(f"{s['label']:<20} | {r21:<8} | {r22:<8} | {r23:<8} | {r24:<8} | {r25:<11} | {r26:<10}")
        
    print("\n" + "=" * 115)
    print("TOP-2 (90/10) ALFA ATTRIBUTION OTOPSISI (NEREDEN KAZANDIRDI / NEREDEN KAYBETTIRDI?):")
    print("=" * 115)
    top2_9010 = sonuclar[1]
    print(f"  • Top-2 Sayesinde Ekstra Kazanılan Tutar : +{top2_9010['top2_ekstra_kazanc_tl']:,.2f} TL (2. fonun 1. fonu geçtiği günler)")
    print(f"  • Top-2 Yüzünden Kaybedilen Tutar (Fırsat) : -{top2_9010['top2_kayip_tl']:,.2f} TL (1. fonun 2. fondan hızlı gittiği günler)")
    print(f"  • NET TOP-2 ALFA KATKISI                   : {top2_9010['net_top2_alfa_tl']:>+12,.2f} TL")
    print("=" * 115)

if __name__ == "__main__":
    run_parity_and_attribution_audit()
