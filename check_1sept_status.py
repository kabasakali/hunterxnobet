...
fonlar = ['SOS', 'MTD', 'CKL', 'DGF', 'GPG', 'PBR', 'PPZ', 'URA', 'ICH', 'TI3']

print("=== 1 EYLUL 2026 TEFAS CANLI FIYAT VE GETIRILER ===")
for f in fonlar:
    try:
        r = requests.post(url, json={'fonKodu': f, 'dil': 'TR'}, timeout=5)
...
for f, info in durum.get("eldeki_fonlar", {}).items():
    print(f"  - {f}: {info.get('pay_adedi'):,.2f} Pay | Maliyet: {info.get('maliyet_fiyati'):.6f} | Son NAV: {info.get('son_nav'):.6f} | Tutar: {info.get('guncel_deger_tl'):,.2f} TL | Kar: {info.get('guncel_deger_tl', 0) - info.get('pay_adedi', 0)*info.get('maliyet_fiyati', 0):,.2f} TL")
